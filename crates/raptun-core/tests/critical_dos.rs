...
async fn server_clamps_malicious_block_size() {
    ...
    let _server = tokio::spawn(async move {
        ...
        let ep = raptun_core::endpoint::build_server_endpoint(server_addr, &identity, transport)
            .unwrap();
    ...
    let client_ep = raptun_core::endpoint::build_client_endpoint(&trust, transport).unwrap();
    let conn = client_ep
