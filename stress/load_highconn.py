#!/usr/bin/env python3
"""High-concurrency load driver: 1–10 MB payloads, reconnect after each transfer.
...
# ---------------------------------------------------------------------------
# Load driver: each worker sends one request → verifies → reconnects → repeats.
# ---------------------------------------------------------------------------
MB = 1024 * 1024
    ...
    def worker(wid: int):
        ...
        while time.time() < stop_at:
            try:
                ...
                mb = sizes[rng.randint(0, len(sizes) - 1)]
                payload = payloads[mb]
                seq += 1
                t0 = time.time()
                ...
                ok = (rn == len(payload) and rck == checksum(payload))
                with lock:
                    if ok:
                        stats["requests_ok"] += 1
